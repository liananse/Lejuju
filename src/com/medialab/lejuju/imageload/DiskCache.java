...
import com.medialab.lejuju.imageload.BytesBufferPool.BytesBuffer;
...
//            long candidateKey = mIndexBuffer.getLong(offset);
    ...
    public void addToDiskCache(String url, byte[] data) {
        if (this == null || url == null || data == null) {
            return;
        }
        //Add to disk cache
        ...
        synchronized (this) {
            try {
            	this.insert(cacheKey, buffer.array());
            } catch (IOException ex) {
                // ignore.
            }
        }
    ...
    public Bitmap getFromDisk(String key) {
        ...
        Bitmap b = null;
        try {
        	boolean found = getImageData(key, buffer);
