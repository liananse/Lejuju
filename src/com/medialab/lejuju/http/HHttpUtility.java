		...
		try 
		{
			...
			if (keySet != null)
			{
				for (Iterator<String> iterator = keySet.iterator(); iterator.hasNext();)
				{
					...
					if (key.equals("image") || key.equals("image1"))
					{
						file = params.get(key);
					}
				}
...
//			bos.write(data);
	...
	public static void appendBasicParams(Context context, Map<String, String> params)
	{
		...
		params.put("softversion", UTools.OS.getAppVersion(context));
		params.put("channel", "medialab");
		// params.put("origin", value);
		// params.put("phonecode", value);
		// language not need Temporarily
		// params.put("language", UTools.OS.getLanguage());
		params.put("countrycode", UTools.OS.getCountryCode());
		params.put("app_name", UTools.OS.getAppName());
	...
	private static void paramToUpload(OutputStream baos, Map<String, String> params) throws UException
	{
		...
		if (keySet != null)
		{
			for (Iterator<String> iterator = keySet.iterator(); iterator.hasNext();)
			{
				...
				if (key1.equals("image") || key1.equals("image1"))
				{
					continue;
				}
